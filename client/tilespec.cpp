...
#define SET_SPRITE(field, tag)                                              \
  do {                                                                      \
    ...
    if (t->sprites.field == NULL) {                                         \
      tileset_error(LOG_FATAL, _("Sprite for tag '%s' missing."), qUtf8Printable(tag));     \
    }                                                                       \
  } while (FALSE)
...
#define SET_SPRITE_ALT_OPT(field, tag, alt)                                 \
  do {                                                                      \
    t->sprites.field = tiles_lookup_sprite_tag_alt(                         \
        t, LOG_VERBOSE, qUtf8Printable(tag), qUtf8Printable(alt), "sprite", #field, TRUE);                  \
  } while (FALSE)
 ...
 ****************************************************************************/
static int
load_city_thresholds_sprites(struct tileset *t, const char *tag,
...
#undef SET_GOTO_TURN_SPRITE
  ...
  output_type_iterate(o)
  {
    buffer = QString("upkeep.%1").arg(get_output_identifier(static_cast<Output_type_id>(o)));
    SET_SPRITE_OPT(upkeep.output[o][0], buffer);
    for (i = 1; i < MAX_NUM_UPKEEP_SPRITES; i++) {
      buffer2 = QString("upkeep.%1%2").arg(get_output_identifier(static_cast<Output_type_id>(o)), QString::number(i+1));
      if (sprite_exists(t, buffer2)) {
                               ...
                               const char *tag_pfx)
{
  ...
  for (i = 0; i < t->num_index_cardinal; i++) {
    fc_snprintf(buffer, sizeof(buffer), "%s_s_%s", tag_pfx,
  ...
  for (i = 0; i < t->num_cardinal_tileset_dirs; i++) {
    fc_snprintf(buffer, sizeof(buffer), "%s_outlet_%s", tag_pfx,
...
void tileset_setup_extra(struct tileset *t, struct extra_type *pextra)
{
  ...
  if (!fc_strcasecmp(pextra->graphic_str, "none")) {
    ...
    case ESTYLE_CARDINALS: {
       ...
       * graphics. */
      for (i = 0; i < t->num_index_cardinal; i++) {
        fc_snprintf(buffer, sizeof(buffer), "%s_%s", tag,
                    cardinal_index_str(t, i));
                             ...
                             const struct terrain *pterrain)
{
  ...
  for (l = 0; l < draw->num_layers; l++) {
    ...
    case CELL_WHOLE:
      ...
      case MATCH_NONE:
        ...
        for (i = 0;; i++) {
          fc_snprintf(buffer, sizeof(buffer), "t.l%d.%s%d", l, draw->name,
                      i + 1);
      ...
      case MATCH_SAME:
        ...
        for (i = 0; i < t->num_index_cardinal; i++) {
          fc_snprintf(buffer, sizeof(buffer), "t.l%d.%s_%s", l, draw->name,
                      cardinal_index_str(t, i));
          dlp->match[i] = tiles_lookup_sprite_tag_alt(
              t, LOG_FATAL, buffer, "", "matched terrain",
    ...
    case CELL_CORNER: {
      ...
      for (i = 0; i < number; i++) {
        ...
        switch (dlp->match_style) {
        case MATCH_NONE:
          fc_snprintf(buffer, sizeof(buffer), "t.l%d.%s_cell_%c", l,
                      draw->name, direction4letters[dir]);
          dlp->cells[i] = tiles_lookup_sprite_tag_alt(
              t, LOG_FATAL, buffer, "", "cell terrain",
              terrain_rule_name(pterrain), TRUE);
          break;
        case MATCH_SAME:
          fc_snprintf(buffer, sizeof(buffer), "t.l%d.%s_cell_%c%d%d%d", l,
                      draw->name, direction4letters[dir], (value) &1,
                      (value >> 1) & 1, (value >> 2) & 1);
          dlp->cells[i] = tiles_lookup_sprite_tag_alt(
              t, LOG_FATAL, buffer, "", "same cell terrain",
              terrain_rule_name(pterrain), TRUE);
          break;
        case MATCH_PAIR:
          fc_snprintf(
              buffer, sizeof(buffer), "t.l%d.%s_cell_%c_%c_%c_%c", l,
              draw->name, direction4letters[dir],
          ...
          dlp->cells[i] = tiles_lookup_sprite_tag_alt(
              t, LOG_FATAL, buffer, "", "cell pair terrain",
              terrain_rule_name(pterrain), TRUE);
          break;
        case MATCH_FULL: {
          ...
          fc_snprintf(buffer, sizeof(buffer), "t.l%d.cellgroup_%c_%c_%c_%c",
                      l, tslp->match_types[n][0], tslp->match_types[e][0],
  ...
  draw->blender = tiles_lookup_sprite_tag_alt(
      t, LOG_VERBOSE, buffer, "", "blend terrain",
      terrain_rule_name(pterrain), TRUE);
  ...
  if (draw->blending > 0) {
    ...
    if (NULL == draw->blender) {
      ...
      draw->blender = tiles_lookup_sprite_tag_alt(
          t, LOG_FATAL, buffer, "", "base (blend) terrain",
          terrain_rule_name(pterrain), TRUE);
    }
  }
                                  ...
                                  bool backdrop)
{
...

  {
    ...
    if (s != NULL) {
      ADD_SPRITE(s, TRUE, FULL_TILE_X_OFFSET + offset_x,
                                  ...
                                  const struct city *pcity)
{
  ...
  for (dir = 0; dir < 8; dir++) {
     ...
     * has road, so it will have road connection with any adjacent road tile.
     * We check from real existence of road (road_near[dir]) and not from
     * whether road gets drawn (draw_road[dir]) as latter can be FALSE when
     * road is simply hidden by another one, and we don't want to
  ...
  if (extrastyle == ESTYLE_ROAD_ALL_SEPARATE) {
    ...
    if (road) {
      ...
      if (road_even_tileno != 0) {
        ADD_SPRITE_SIMPLE(t->sprites.extras[extra_idx]
  ...
  case DARKNESS_ISORECT:
    for (i = 0; i < 4; i++) {
      ...
      if (UNKNOWN(static_cast<direction8>(DIR4_TO_DIR8[i]))) {
        ADD_SPRITE(t->sprites.tx.darkness[i], TRUE, offsets[i][0],
                   offsets[i][1]);
      }
    }
    break;
                      ...
                      const struct unit_type *putype)
{
  ...
  case LAYER_CITY1:
    ...
    if (pcity && gui_options.draw_cities) {
      ...
      if (t->type == TS_ISOMETRIC && pcity->client.walls > 0) {
        ...
        if (spr != NULL) {
          ADD_SPRITE(spr, TRUE, FULL_TILE_X_OFFSET + t->city_offset_x,
                     FULL_TILE_Y_OFFSET + t->city_offset_y);
        }
      }
    ...
    if (NULL != pterrain) {
      if (ptile) {
        ...
        if (eowner != NULL) {
          ...
          if (show_flag) {
            ADD_SPRITE(get_nation_flag_sprite(t, nation_of_player(eowner)),
  ...
  case LAYER_CITY2:
    ...
    if (pcity && gui_options.draw_cities && !gui_options.draw_full_citybar) {
      ...
      if (10 <= size) {
        ADD_SPRITE(t->sprites.city.size_tens[(size / 10) % 10], FALSE,
                   FULL_TILE_X_OFFSET + t->city_size_offset_x,
                   FULL_TILE_Y_OFFSET + t->city_size_offset_y);
        if (100 <= size) {
          struct sprite *sprite =
    ...
    if (citymode != NULL && ptile != NULL) {
      worker_task_list_iterate(citymode->task_reqs, ptask)
      {
        if (ptask->ptile == ptile) {
          ...
          case ACTIVITY_MINE:
            if (ptask->tgt == NULL) {
              ADD_SPRITE(t->sprites.unit.plant, TRUE,
                         FULL_TILE_X_OFFSET + t->activity_offset_x,
                         FULL_TILE_Y_OFFSET + t->activity_offset_y);
            } else {
          ...
          case ACTIVITY_IRRIGATE:
            if (ptask->tgt == NULL) {
              ADD_SPRITE(t->sprites.unit.irrigate, TRUE,
                         FULL_TILE_X_OFFSET + t->activity_offset_x,
                         FULL_TILE_Y_OFFSET + t->activity_offset_y);
            } else {
              ADD_SPRITE(t->sprites.extras[extra_index(ptask->tgt)].activity,
          ...
          case ACTIVITY_GEN_ROAD:
            ...
            break;
          case ACTIVITY_TRANSFORM:
            ADD_SPRITE(t->sprites.unit.transform, TRUE,
                       FULL_TILE_X_OFFSET + t->activity_offset_x,
                       FULL_TILE_Y_OFFSET + t->activity_offset_y);
            break;
          case ACTIVITY_POLLUTION:
  ...
  case LAYER_INFRAWORK:
    if (ptile != NULL && ptile->placing != NULL) {
      ...
      if (t->sprites.extras[id].activity != NULL) {
        ADD_SPRITE(t->sprites.extras[id].activity, TRUE,
                   FULL_TILE_X_OFFSET + t->activity_offset_x,
                   FULL_TILE_Y_OFFSET + t->activity_offset_y);
      }
    }
