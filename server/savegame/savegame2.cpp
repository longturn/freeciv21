            ...
            extra_type_by_cause_iterate(cause, candidate)
            {
              if (!tile_has_extra(vtile, candidate)) {
                if ((!is_extra_caused_by(candidate, EC_BASE)
                    ...
                    && are_reqs_active(NULL, tile_owner(vtile), NULL, NULL,
                                       vtile, NULL, NULL, NULL, NULL, NULL,
...
static void sg_load_random(struct loaddata *loading)
{
  ...
  if (secfile_lookup_bool_default(loading->file, FALSE, "random.saved")) {
    ...
    for (i = 0; i < 8; i++) {
      ...
      sscanf(string, "%8x %8x %8x %8x %8x %8x %8x",
             &loading->rstate.v[7 * i], &loading->rstate.v[7 * i + 1],
             &loading->rstate.v[7 * i + 2], &loading->rstate.v[7 * i + 3],
             &loading->rstate.v[7 * i + 4], &loading->rstate.v[7 * i + 5],
             &loading->rstate.v[7 * i + 6]);
    }
                                ...
                                struct unit *punit, const char *unitstr)
{
  ...
  /* load the unit orders */
  {
    ...
    if (len > 0) {
      ...
      tgt_unitstr = secfile_lookup_str_default(loading->file, NULL,
                                               "%s.tgt_list", unitstr);

      if (tgt_unitstr == NULL) {
        base_unitstr =
            secfile_lookup_str(loading->file, "%s.base_list", unitstr);
        road_unitstr = secfile_lookup_str_default(loading->file, NULL,
                                                  "%s.road_list", unitstr);
      }
          ...
          if (base_unitstr && base_unitstr[j] != '?'
            ...
            if (base_id < 0 || base_id >= loading->base.size) {
              log_sg("Cannot find base %d for %s to build", base_id,
                     unit_rule_name(punit));
              base_id = base_number(
                  get_base_by_gui_type(BASE_GUI_FORTRESS, NULL, NULL));
            }
