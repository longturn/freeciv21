...
static void sg_load_savefile(struct loaddata *loading)
{
  ...
  if (loading->special.size) {
    ...
    loading->special.order = static_cast<tile_special_type*>(fc_calloc(nmod,
  ...
  if (loading->base.size) {
    ...
    loading->base.order = static_cast<base_type**>(fc_calloc(nmod, sizeof(*loading->base.order)));
  ...
  if (loading->road.size) {
    ...
    loading->road.order = static_cast<road_type**>(fc_calloc(nmod, sizeof(*loading->road.order)));
  ...
  if (loading->ds_t.size) {
    ...
    loading->ds_t.order = static_cast<diplstate_type*>(fc_calloc(loading->ds_t.size,
...
static void sg_load_game(struct loaddata *loading)
{
  ...
  game.info.phase_mode
    = static_cast<phase_mode_type>(secfile_lookup_int_default(loading->file, GAME_DEFAULT_PHASE_MODE,
                                 "game.phase_mode"));
                                ...
                                const char *unitstr)
{
  ...
  punit->action_decision_want
      = static_cast<action_decision>(secfile_lookup_enum_default(loading->file,
    ...
    halfbyte_iterate_special(x, loading->special.size) {
      LOAD_MAP_CHAR(ch, ptile,
                    sg_special_set(ptile, &map_get_player_tile(ptile, plr)->extras,
                                   ch, loading->special.order + 4 * static_cast<int>(x), FALSE),
