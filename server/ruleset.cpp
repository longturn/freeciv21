                        ...
                        enum log_level level, const char *format, ...)
{
  ...
  vdo_log(file, function, line, FALSE, level, buf, sizeof(buf), format,
          args);
  va_end(args);
                        ...
                        const char *filename, const char *description)
{
  ...
  if (!sval || !strcmp(sval, "Never")) {
    ...
    *result = advance_by_rule_name(sval);
    ...
    if (A_NEVER == *result) {
      ruleset_error(LOG_ERROR, "\"%s\" %s %s: couldn't match \"%s\".",
                             ...
                             const char *filename)
{
  ...
  if (ok) {
    for (i = 0; i < nval; i++) {
      ...
      if (!punittype) {
        ruleset_error(LOG_ERROR, "\"%s\" %s.%s (%d): couldn't match \"%s\".",
                                 ...
                                 int *output, const char *filename)
{
  ...
  if (nval > MAX_NUM_BUILDING_LIST) {
    ruleset_error(
        LOG_ERROR, "\"%s\": string vector %s.%s too long (%d, max %d)",
                                            ...
                                            struct government *fallback)
{
  ...
  if (!gov) {
    ruleset_error(LOG_ERROR, "\"%s\" %s: couldn't match \"%s\".", filename,
                  entry, sval);
  }
  return gov;
}
...
lookup_resource(const char *filename, const char *name, const char *jsection)
{
  ...
  if (pres == NULL) {
    ruleset_error(LOG_ERROR, "\"%s\" [%s] has unknown \"%s\".", filename,
                  jsection, name);
  }
                               ...
                               const char *sec_name)
{
  ...
  if (!name) {
    ruleset_error(LOG_ERROR, "\"%s\" [%s]: no \"name\" specified.",
                            ...
                            struct rscompat_info *compat)
{
  ...
  for (i = 0; (flag = secfile_lookup_str_default(file, NULL,
                                                 "control.flags%d.name", i));
       i++) {
    const char *helptxt =
        secfile_lookup_str_default(file, NULL, "control.flags%d.helptxt", i);
  ...
  advance_iterate(A_FIRST, a)
  {
    ...
    if (game.control.num_tech_classes == 0) {
      ...
      if (classname != NULL) {
        ...
        if (a->tclass == NULL) {
          ruleset_error(
              LOG_ERROR, "\"%s\" [%s] \"%s\": Uknown tech class \"%s\".",
                            ...
                            struct rscompat_info *compat)
{
  ...
  if (ok) {
    ...
    for (i = 0; (flag = secfile_lookup_str_default(
                     file, NULL, "control.class_flags%d.name", i));
         i++) {
      const char *helptxt = secfile_lookup_str_default(
          file, NULL, "control.class_flags%d.helptxt", i);
                               ...
                               struct rscompat_info *compat)
{
  ...
  for (i = 0; (flag = secfile_lookup_str_default(
                   file, NULL, "control.extra_flags%d.name", i));
       i++) {
    const char *helptxt = secfile_lookup_str_default(
        file, NULL, "control.extra_flags%d.helptxt", i);
  ...
  if (ok) {
    ...
    for (; i < MAX_NUM_USER_EXTRA_FLAGS; i++) {
      set_user_extra_flag_name(extra_flag_id(EF_USER_FLAG_1 + i), NULL,
                               NULL);
    }
  ...
  if (ok) {
    ...
    for (idx = 0; idx < nval; idx++) {
      ...
      resource_name =
          secfile_lookup_str_default(file, NULL, "%s.extra", sec_name);
      ...
      extra_type_by_cause_iterate(EC_RESOURCE, pres2)
      {
        if (pres->data.resource->id_old_save
          ...
          ruleset_error(
              ...
              filename, extra_rule_name(pres), extra_rule_name(pres2));
          ok = FALSE;
          break;
        }
      }
  ...
  if (NULL == sec) {
    ...
    nations_iterate(pl)
    {
      ...
      const char *domain = secfile_lookup_str_default(
          file, NULL, "%s.translation_domain", sec_name);
                                 ...
                                 struct rscompat_info *compat)
{
  ...
  if (ok) {
    ...
    if (sec) {
      ...
      ruleset_error(LOG_ERROR,
                    "At least one nation set [" NATION_SET_SECTION_PREFIX
                    "_*] "
                    "must be defined.");
      ok = FALSE;
    }
  ...
  if (ok) {
    ...
    if (sval != NULL) {
      ...
      if (pset != NULL) {
        ...
        ruleset_error(LOG_ERROR, "Unknown default_nationset \"%s\".", sval);
        ok = FALSE;
      }
    ...
    nations_iterate(pnation)
    {
      ...
      for (j = 0;; j++) {
        ...
        name = secfile_lookup_str_default(
            file, NULL, "%s.ruler_titles%d.government", sec_name, j);
        if (NULL == name) {
      ...
      if (secfile_entry_by_path(file, tmp)) {
        pnation->init_government =
            lookup_government(file, tmp, filename, NULL);
      ...
      if (check_strlen(pnation->legend, MAX_LEN_MSG, NULL)) {
        ruleset_error(LOG_ERROR, "Nation %s: legend \"%s\" is too long.",
                                         ...
                                         const char *filename)
{
  ...
  if (secfile_entry_lookup(file, "%s", uflags_path)) {
    ...
    if (!protecor_flag) {
      ...
      ruleset_error(LOG_ERROR, "\"%s\": %s: bad unit type flag list.",
                                     ...
                                     const char *filename)
{
  ...
  if (secfile_entry_lookup(file, "%s", actions_path)) {
    ...
    if (!unit_acts) {
      /* Entity exists but couldn't read it. */
      ruleset_error(LOG_ERROR, "\"%s\": %s: bad action list", filename,
                    actions_path);
                              ...
                              struct rscompat_info *compat)
{
  ...
  if (ok) {
    ...
    if (!gold_upkeep_style_is_valid(game.info.gold_upkeep_style)) {
      ruleset_error(LOG_ERROR, "Unknown gold upkeep style \"%s\"", tus_text);
      ok = FALSE;
    }
    ...
    if (ok) {
      ...
      if (!load_action_auto_actions(
        ...
        ruleset_error(LOG_ERROR, "\"%s\": %s: failed load %s.", filename,
      ...
      if (compat->compat_mode) {
        ...
        if (!rscompat_auto_attack_3_1(compat, auto_perf, psize,
                                      protecor_flag)) {
          /* Upgrade failed */
          ruleset_error(LOG_ERROR, "\"%s\": %s: failed to upgrade.",
  ...
  if (ok) {
    ...
    if (!tech_cost_style_is_valid(game.info.tech_cost_style)) {
      ruleset_error(LOG_ERROR, "Unknown tech cost style \"%s\"", tus_text);
      ok = FALSE;
    }
    ...
    if (!tech_leakage_style_is_valid(game.info.tech_leakage)) {
      ruleset_error(LOG_ERROR, "Unknown tech leakage \"%s\"", tus_text);
      ok = FALSE;
    }
    ...
    if (!tech_upkeep_style_is_valid(game.info.tech_upkeep_style)) {
      ruleset_error(LOG_ERROR, "Unknown tech upkeep style \"%s\"", tus_text);
      ok = FALSE;
    }
  }
  ...
  if (ok) {
    ...
    achievements_iterate(pach)
    {
      ...
      type_name =
          secfile_lookup_str_default(file, NULL, "%s.type", sec_name);
      ...
      if (ok) {
        ...
        msg =
            secfile_lookup_str_default(file, NULL, "%s.first_msg", sec_name);
        if (msg == NULL) {
          ruleset_error(LOG_ERROR, "Achievement %s has no first msg!",
                        sec_name);
          ok = FALSE;
        } else {
      ...
      if (ok) {
        msg =
            secfile_lookup_str_default(file, NULL, "%s.cons_msg", sec_name);
        if (msg == NULL) {
          if (!pach->unique) {
            ruleset_error(
                LOG_ERROR,
  ...
  if (ok) {
    for (i = 0; (name = secfile_lookup_str_default(
                     file, NULL, "trade.settings%d.type", i));
         i++) {
      enum trade_route_type type = trade_route_type_by_name(name);

      if (type == TRT_LAST) {
        ruleset_error(LOG_ERROR, "\"%s\" unknown trade route type \"%s\".",
                      filename, name);
        ok = FALSE;
      } else {
...
static void send_ruleset_terrain(struct conn_list *dest)
{
  ...
  terrain_type_iterate(pterrain)
  {
    ...
    packet.animal =
        (pterrain->animal == NULL ? -1 : utype_number(pterrain->animal));
                            ...
                            bool buffer_script, bool load_luadata)
{
  ...
  if (techfile == NULL || buildfile == NULL || govfile == NULL
