...
int main(int argc, char *argv[])
{
  ...
  inx = 1;
  while (inx < argc) {
    if ((option = get_option_malloc("--file", argv, &inx, argc, FALSE))) {
      ...
      srvarg.metaserver_no_send = FALSE;
    } else if ((option = get_option_malloc("--Metaserver", argv, &inx, argc,
                                           FALSE))) {
      sz_strlcpy(srvarg.metaserver_addr, option);
      free(option);
      srvarg.metaserver_no_send = FALSE; /* --Metaserver implies --meta */
    } else if ((option = get_option_malloc("--identity", argv, &inx, argc,
                                           FALSE))) {
      sz_strlcpy(srvarg.identity_name, option);
      free(option);
    } else if ((option =
                    get_option_malloc("--port", argv, &inx, argc, FALSE))) {
      ...
      free(option);
                    ...
                    get_option_malloc("--bind", argv, &inx, argc, TRUE))) {
      ...
      srvarg.bind_meta_addr = option;
                    ...
                    get_option_malloc("--type", argv, &inx, argc, FALSE))) {
      sz_strlcpy(game.server.meta_info.type, option);
      free(option);
#endif /* FREECIV_WEB */
    } else if ((option =
                    get_option_malloc("--read", argv, &inx, argc, TRUE)))
      ...
      free(option);
                    ...
                    get_option_malloc("--saves", argv, &inx, argc, TRUE))) {
  ...
  if (showhelp) {
...
#ifdef HAVE_FCDB
    cmdhelp_add(help, "D",
                /* TRANS: "Database" is exactly what user must type, do not
                   translate. */
                _("Database FILE"),
    ...
    cmdhelp_add(
        ...
        _("bind ADDR"), _("Listen for clients on ADDR"));
    ...
    cmdhelp_add(help, "M",
                /* TRANS: "Metaserver" is exactly what user must type, do not
                   translate. */
                _("Metaserver ADDR"), _("Set ADDR as metaserver address"));
#ifdef FREECIV_WEB
    cmdhelp_add(
        help, "t",
        /* TRANS: "type" is exactly what user must type, do not translate. */
        _("type TYPE"), _("Set TYPE as server type in metaserver"));
#endif /* FREECIV_WEB */
    cmdhelp_add(help, "k", "keep",
        ...
        cmdhelp_add(help, "p",
                    /* TRANS: "port" is exactly what user must type, do not
                       translate. */
                    _("port PORT"), _("Listen for clients on port PORT"));
    cmdhelp_add(help, "q",
                /* TRANS: "quitidle" is exactly what user must type, do not
                   translate. */
                _("quitidle TIME"),
    ...
    cmdhelp_add(help, "s",
                /* TRANS: "saves" is exactly what user must type, do not
                   translate. */
                _("saves DIR"), _("Save games to directory DIR"));
    cmdhelp_add(help, NULL,
                /* TRANS: "scenarios" is exactly what user must type, do not
                   translate. */
                _("scenarios DIR"), _("Save scenarios to directory DIR"));
    cmdhelp_add(help, "S",
                /* TRANS: "Serverid" is exactly what user must type, do not
                   translate. */
                _("Serverid ID"), _("Sets the server id to ID"));
    cmdhelp_add(
        help, "r",
        /* TRANS: "read" is exactly what user must type, do not translate. */
        _("read FILE"), _("Read startup script FILE"));
    cmdhelp_add(help, "R",
                /* TRANS: "Ranklog" is exactly what user must type, do not
                   translate. */
                _("Ranklog FILE"), _("Use FILE as ranking logfile"));
    cmdhelp_add(help, NULL,
                /* TRANS: "ruleset" is exactly what user must type, do not
                   translate. */
                _("ruleset RULESET"), _("Load ruleset RULESET"));
#ifdef AI_MODULES
    cmdhelp_add(help, "L",
...
#define HARDCODED_OPT
  ...
  if (argc == 0) {
    ...
    DisposeDialog(
        optptr); /*get rid of the dialog after sorting out the options*/
