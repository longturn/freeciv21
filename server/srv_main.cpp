    ...
    players_iterate(plr2)
    {
      ...
      if (state->first_contact_turn != game.info.turn) {
        ...
        if (state->type == DS_CEASEFIRE) {
          ...
          case 1:
            notify_player(
                plr1, NULL, E_DIPLOMACY, ftc_server,
...
void end_turn()
{
  ...
  achievements_iterate(ach)
  {
    ...
    if (first != NULL) {
      notify_player(first, NULL, E_ACHIEVEMENT, ftc_server, "%s",
    ...
    if (!ach->unique) {
      player_list_iterate(achievers, pplayer)
      {
        ...
        if (pplayer != first) {
          notify_player(pplayer, NULL, E_ACHIEVEMENT, ftc_server, "%s",
                              ...
                              const char *name, int style)
{
  ...
  if (new_nation != NO_NATION_SELECTED) {
    ...
    if (!server_player_set_name_full(pc, pplayer, new_nation, name, message,
                                     sizeof(message))) {
      notify_player(pplayer, NULL, E_NATION_SELECTED, ftc_server, "%s",
                    message);
      return;
    }
    ...
    server_player_set_name_full(pc, pplayer, NULL, ANON_PLAYER_NAME, message,
                                sizeof(message));
  }
...
static void generate_players(void)
{
  ...
  if (0 < nations_to_assign && 0 < map_startpos_count()) {
    ...
    players_iterate(pplayer)
    {
      ...
      allowed_nations_iterate(pnation)
      {
        if (!is_nation_playable(pnation) || NULL != pnation->player) {
          /* Not available. */
          continue;
        }
    ...
    for (i = 0; !created && i < max; i++) {
      ...
      if (!created && max > 1) {
        ...
        for (set = 0; set < ARRAY_SIZE(mapgen_settings); set++) {
          ...
          fc_assert_action(pset != NULL, continue);
          success = setting_enum_set(pset, mapgen_settings[set].value, NULL,
                                     error, sizeof(error));
