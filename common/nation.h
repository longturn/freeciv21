...
enum nation_city_preference { NCP_DISLIKE = -1, NCP_NONE = 0, NCP_LIKE = 1 };
...
struct nation_type {
  ...
  struct government
      *init_government; /* use game default_government if NULL */
  ...
  union {
    ...
    struct {
       ...
       * in some circumstances the server may decide to put a nation with
       * this flag in the game anyway, so the client can't rely on its
       * absence. (On the server this is calculated on the fly from other
       * values. Use is_nation_pickable() to get the answer on client or
       * server.) */
      bool is_pickable;
    } client;
  };
...
const char *
nation_group_untranslated_name(const struct nation_group *pgroup);
...
#define nation_sets_iterate(NAME_pset)                                      \
  generic_iterate(struct nation_set_iter, struct nation_set *, NAME_pset,   \
                  nation_set_iter_sizeof, nation_set_iter_init)
