...
struct ai_type {
  ...
  struct {
    ...
    /* Called for every AI type when new city is removed from game. Called
     * even for virtual cities. */
    void (*city_free)(struct city *pcity);

    /* Called for every AI type when new city is added to game. Called for
     * real cities only. */
    ...
    /* Called for player AI when building advisor should set wants for
     * buildings. Without this implemented in AI type building advisor does
     * not adjust wants at all. */
    ...
    /* Called for every AI type when unit is removed from game. Called even
     * for virtual units. */
    void (*unit_free)(struct unit *punit);

    /* Called for every AI type when new unit is added to game. Called for
     * real units only. */
    void (*unit_created)(struct unit *punit);

    /* Called for every AI type when unit is removed from game. Called for
     * real units only. */
    ...
    /* Called for player AI type when autosettlers have been handled for the
     * turn. */
    ...
    /* Called for player AI when player phase is already active when AI gains
     * control. */
    void (*restart_phase)(struct player *pplayer);

    /* Called for player AI type in the beginning of player phase. Not for
     * barbarian players. */
    ...
    /* Called for player AI type when first contact with another player has
     * been established. Note that when contact is between two AI players,
     * callback might be already called for the other party, so you can't
     * assume relations to be all-pristine when this gets called. */
    void (*first_contact)(struct player *pplayer, struct player *aplayer);

    /* Called for player AI type of the victim when someone does some
     * violation against him/her. */
    void (*incident)(enum incident_type type, enum casus_belli_range scope,
                     const struct action *paction, struct player *receiver,
                     struct player *violator, struct player *victim);

    /* Called for player AI type of city owner when logging requires city
     * debug information. */
    void (*log_fragment_city)(char *buffer, int buflength,
                              const struct city *pcity);

    /* Called for player AI type of unit owner when logging requires unit
     * debug information. */
    ...
    /* Called for player AI type to decide if it's dangerous for unit to
     * enter tile. */
    void (*consider_tile_dangerous)(struct tile *ptile, struct unit *punit,
                                    enum override_bool *result);

    /* Called for player AI to decide if city can be chosen to act as wonder
     ...
     * place. Currently this gets called when:
     *  - city changes owner.
     */
    void (*city_info)(struct city *pcity);

    /* Called for every AI type when certain kind of unit change has taken
     * place. Currently this gets called when:
